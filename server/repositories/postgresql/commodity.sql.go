...
const insertDailyCommodity = `-- name: InsertDailyCommodity :exec
INSERT INTO daily_commodities (id, commodities, publish_date)
VALUES ($1, $2, $3)
`

type InsertDailyCommodityParams struct {
	ID          string      `json:"id"`
	Commodities []byte      `json:"commodities"`
	PublishDate pgtype.Date `json:"publish_date"`
}

func (q *Queries) InsertDailyCommodity(ctx context.Context, arg InsertDailyCommodityParams) error {
	_, err := q.db.Exec(ctx, insertDailyCommodity, arg.ID, arg.Commodities, arg.PublishDate)
	return err
}
